...
use imports::{
    DcdtLocalRole, DrtscPath, TestAddress, TestDcdtTransfer, TestSCAddress, TestTokenIdentifier,
};
use dharitri_sc::types::{BigUint, DcdtTokenPayment, ManagedAddress};
use dharitri_sc_modules::only_admin::OnlyAdminModule;
use dharitri_sc_scenario::*;
use on_chain_claim::*;

const ON_CHAIN_CLAIM_PATH: DrtscPath = DrtscPath::new("drtsc:output/on-chain-claim.drtsc.json");
...
fn setup_with_one_token(world: &mut ScenarioWorld) {
    let roles: Vec<String> = vec![DcdtLocalRole::NftBurn.name().to_string()];

    world.account(OWNER).nonce(1);
    world
        .account(USER)
        .nonce(1)
        .dcdt_nft_balance(XREPAIR_TOKEN, TOKEN_NONCE, 1, ());
    world
        .account(SC_ADDR)
        .nonce(1)
        .code(ON_CHAIN_CLAIM_PATH)
        .owner(OWNER)
        .dcdt_roles(XREPAIR_TOKEN, roles);
...
fn setup_with_two_token(world: &mut ScenarioWorld) {
    ...
    world
        .account(USER)
        .nonce(1)
        .dcdt_nft_balance(XREPAIR_TOKEN, TOKEN_NONCE, 100, ())
        .dcdt_nft_balance(XREPAIR_TOKEN_2, TOKEN_NONCE, 1, ());
    world
        .account(SC_ADDR)
        .nonce(1)
        .code(ON_CHAIN_CLAIM_PATH)
...
fn check_token_identifier() {
    ...
    world
        ...
        .whitebox(on_chain_claim::contract_obj, |sc| {
            let payment = DcdtTokenPayment::new(
...
fn check_claim_and_repair() {
    ...
    world
        ...
        .from(USER)
        .to(SC_ADDR)
        .dcdt(TestDcdtTransfer(XREPAIR_TOKEN, TOKEN_NONCE, 1))
    ...
    world
        ...
        .from(USER)
        .to(SC_ADDR)
        .dcdt(TestDcdtTransfer(XREPAIR_TOKEN_2, TOKEN_NONCE, 1))
    ...
    world
        ...
        .from(USER)
        .to(SC_ADDR)
        .dcdt(TestDcdtTransfer(XREPAIR_TOKEN, TOKEN_NONCE, 2))
        .with_result(ExpectMessage("Bad payment token/amount"))
        .whitebox(on_chain_claim::contract_obj, |sc| {
            sc.claim_and_repair();
        });
