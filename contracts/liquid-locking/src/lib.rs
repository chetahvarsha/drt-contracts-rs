...
pub trait LiquidLocking {
    ...
    fn whitelist_token(&self, token: TokenIdentifier) {
        require!(token.is_valid_dcdt_identifier(), "invalid token provided");
    ...
    fn validate_payment(&self, payment: &DcdtTokenPayment) {
    ...
    fn stake_token(&self, caller: &ManagedAddress, payment: DcdtTokenPayment) {
    ...
    fn unbond(&self, tokens: ManagedVec<TokenIdentifier>) {
        let caller = self.blockchain().get_caller();
        let block_epoch = self.blockchain().get_block_epoch();
        let mut unbond_tokens = ManagedVec::<Self::Api, DcdtTokenPayment<Self::Api>>::new();
        for token_identifier in tokens.iter() {
            ...
            if unbond_amount > 0u64 {
                unbond_tokens.push(DcdtTokenPayment::new(
    ...
    #[event("lock")]
    fn lock_event(&self, #[indexed] user: &ManagedAddress, tokens: &ManagedVec<DcdtTokenPayment>);

    #[event("unlock")]
    fn unlock_event(&self, #[indexed] user: &ManagedAddress, tokens: &ManagedVec<DcdtTokenPayment>);

    #[event("unbond")]
    fn unbond_event(&self, #[indexed] user: &ManagedAddress, tokens: &ManagedVec<DcdtTokenPayment>);
    ...
    fn locked_token_amounts_by_address(
        ...
        let mut amounts = ManagedVec::<Self::Api, DcdtTokenPayment<Self::Api>>::new();
        for token in self.locked_tokens(&address).iter() {
            let amount = self.locked_token_amounts(&address, &token).get();
            let payment = DcdtTokenPayment::new(token, 0, amount);
    ...
    fn unlocked_token_by_address(
        ...
        for token in self.unlocked_tokens(&address).iter() {
            for epoch in self.unlocked_token_epochs(&address, &token).iter() {
                ...
                let payment = DcdtTokenPayment::new(token.clone(), 0, amount);
