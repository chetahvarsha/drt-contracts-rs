    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    fn buy_token(
        ...
        let all_transfers = self
            ...
            .sync_call();
        let received_tokens = all_transfers.dcdt_payments.get(0);
    ...
    fn burn_tokens(&self, tokens: &DcdtTokenPayment) {
        let token_roles = self
            .blockchain()
            .get_dcdt_local_roles(&tokens.token_identifier);
        if token_roles.has_role(&DcdtLocalRole::Burn) {
