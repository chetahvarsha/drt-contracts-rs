...
build_ping_pong() {
    (set -x; drtpy --verbose contract build "$PING_PONG_CONTRACT")
}

deploy_ping_pong() {
    local FIXED_SUM=1500000000000000000 # 1.5 REWA
    local DURATION=$1
    ...
    (set -x; drtpy contract deploy --bytecode="$PING_PONG_CONTRACT/output/ping-pong-rewa.drtsc.json" \
        --pem="$PEM_FILE" \
