...
use proxy_ping_pong_rewa::{ContractState, UserStatus};

const INTERACTOR_SCENARIO_TRACE_PATH: &str = "interactor_trace.scen.json";

const PING_PONG_CODE: DrtscPath = DrtscPath::new("../output/ping-pong-rewa.drtsc.json");

pub async fn ping_pong_rewa_cli() {
    env_logger::init();

    let config = Config::load_config();

    let mut interact = PingPongRewaInteract::init(config).await;
...
pub struct PingPongRewaInteract {
...
impl PingPongRewaInteract {
    pub async fn init(config: Config) -> Self {
        ...
        interactor.set_current_dir_from_workspace("contracts/ping-pong-rewa/interactor");
    ...
    pub async fn deploy(
        ...
        let (new_address, status, message) = self
            ...
            .gas(30_000_000u64)
            .typed(proxy_ping_pong_rewa::PingPongProxy)
            .init(
                ping_amount,
                duration_in_seconds,
        ...
        self.state.set_ping_pong_rewa_address(new_address);
    ...
    pub async fn upgrade(
        ...
        let response = self
            .interactor
            .tx()
            .to(self.state.current_ping_pong_rewa_address())
    ...
    pub async fn ping(&mut self, rewa_amount: u64, message: Option<&str>, sender: &Bech32Address) {
