    ...
    + events::EventsModule
{
    ...
    fn init(&self, mystery_box_token_id: TokenIdentifier) {
        require!(
            mystery_box_token_id.is_valid_dcdt_identifier(),
            "Invalid token ID"
        );
    ...
    fn setup_mystery_box(
        ...
        winning_rates_list: MultiValueEncoded<
            MultiValue6<RewardType, RewaOrDcdtTokenIdentifier, BigUint, ManagedBuffer, u64, u64>,
        >,
    ) {
    ...
    fn create_mystery_box(&self, amount: BigUint) -> DcdtTokenPayment<Self::Api> {
        self.require_caller_is_admin();
