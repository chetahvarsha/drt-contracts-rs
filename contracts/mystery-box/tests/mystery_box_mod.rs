use dharitri_sc::{
    imports::MultiValue6,
    types::{
        BigUint, RewaOrDcdtTokenIdentifier, DcdtLocalRole, ManagedBuffer, MultiValueEncoded,
        ReturnsResult, TestAddress, TestDcdtTransfer, TestSCAddress, TestTokenIdentifier,
    },
};
use dharitri_sc_scenario::{
    api::StaticApi, imports::DrtscPath, managed_buffer, ScenarioTxRun, ScenarioTxWhitebox,
    ScenarioWorld,
};
use mystery_box::{mystery_box_proxy, MysteryBox};

pub const CODE_PATH: DrtscPath = DrtscPath::new("output/mystery-box.drtsc.json");
...
impl MysteryBoxSetup {
    pub fn new() -> Self {
        ...
        let mb_token_roles = vec![
            DcdtLocalRole::NftCreate.name().to_string(),
            DcdtLocalRole::NftAddQuantity.name().to_string(),
        ...
        mystery_box
            ...
            .code(CODE_PATH)
            .owner(OWNER)
            .dcdt_roles(MB_TOKEN, mb_token_roles);
        mystery_box
            .world
            .tx()
            .from(OWNER)
    ...
    pub fn setup_mystery_box(
        ...
        reward = (
            ...
            RewaOrDcdtTokenIdentifier::dcdt(MB_TOKEN.to_token_identifier()),
            BigUint::from(1u64),
    ...
    pub fn open_mystery_box(&mut self, mb_token_nonce: u64) {
        self.world
            ...
            .typed(mystery_box_proxy::MysteryBoxProxy)
            .open_mystery_box()
            .dcdt(TestDcdtTransfer(MB_TOKEN, mb_token_nonce, 1))
            .run();
    }
}
