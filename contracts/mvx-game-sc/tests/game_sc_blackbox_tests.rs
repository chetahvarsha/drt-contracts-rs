use imports::{
    DrtscPath, ReturnsResult, ReturnsResultUnmanaged, TestAddress, TestDcdtTransfer, TestSCAddress,
    TestTokenIdentifier,
};
use dharitri_sc::{
...
const GAME_SC_PATH: DrtscPath = DrtscPath::new("output/mvx-game-sc.drtsc.json");
...
impl GameContractState {
    fn new() -> Self {
        ...
        world
            ...
            .nonce(1)
            .balance(BALANCE)
            .dcdt_balance(TOKEN_GAME, BALANCE);
    ...
    fn create_game(
        ...
        self.world
            ...
            .dcdt(TestDcdtTransfer(TOKEN_GAME, 0, STARTING_FEE))
...
fn game_sc_complex_flow() {
    ...
    state.world.transfer_step(
        TransferStep::new()
            .from(OWNER_ADDR)
            .to(GAME_SC_ADDR)
            .dcdt_transfer(TOKEN_GAME.eval_to_expr(), 0, "10_000"),
    );

    state
        .world
        .check_account(GAME_SC_ADDR)
        .dcdt_balance(TOKEN_GAME, 10_320u64);
    ...
    state
        ...
        .dcdt_balance(TOKEN_GAME, 100000140);

    // user2's reward = 60
    state
        .world
        .check_account(USER2_ADDR)
        .dcdt_balance(TOKEN_GAME, 99999960); // balance - wager + 60
}
...
fn invalid_game_test() {
    ...
    state
        .world
        .check_account(USER1_ADDR)
        .dcdt_balance(TOKEN_GAME, 100000000);
