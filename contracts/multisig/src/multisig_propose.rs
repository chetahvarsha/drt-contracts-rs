...
use crate::{
    action::{Action, CallActionData, DcdtTransferExecuteData, GasLimit},
...
pub trait MultisigProposeModule: crate::multisig_state::MultisigStateModule {
    ...
    fn propose_transfer_execute(
        ...
        self.propose_action(Action::SendTransferExecuteRewa(call_data))
    }

    #[endpoint(proposeTransferExecuteDcdt)]
    fn propose_transfer_execute_dcdt(
        &self,
        to: ManagedAddress,
        ...
        let call_data = DcdtTransferExecuteData {
            to,
            tokens,
            opt_gas_limit,
            endpoint_name: function_call.function_name,
        ...
        self.propose_action(Action::SendTransferExecuteDcdt(call_data))
    }
    ...
    fn ensure_valid_transfer_action(&self, action: &Action<Self::Api>) {
        ...
        if let Action::SendTransferExecuteRewa(call_data) = &action {
            let other_sc_shard = self.blockchain().get_shard_of_address(&call_data.to);
            require!(
                call_data.rewa_amount > 0 || !call_data.endpoint_name.is_empty(),
                "proposed action has no effect"
            );
            require!(
        ...
        if let Action::SendTransferExecuteDcdt(call_data) = &action {
