    ...
    crate::multisig_state::MultisigStateModule + crate::multisig_events::MultisigEventsModule
{
    ...
    fn perform_action(&self, action_id: ActionId) -> OptionalValue<ManagedAddress> {
        ...
        match action {
            ...
            Action::SendTransferExecuteRewa(call_data) => {
                let gas = call_data
                    .opt_gas_limit
                    .unwrap_or_else(|| self.ensure_and_get_gas_for_transfer_exec());
                self.perform_transfer_execute_rewa_event(
                    action_id,
                    &call_data.to,
            ...
            Action::SendTransferExecuteDcdt(call_data) => {
                let gas = call_data
                    .opt_gas_limit
                    .unwrap_or_else(|| self.ensure_and_get_gas_for_transfer_exec());

                self.perform_transfer_execute_dcdt_event(
                    action_id,
                    &call_data.to,
                    &call_data.tokens,
                    gas,
                ...
                let result = self.send_raw().multi_dcdt_transfer_execute(
                    &call_data.to,
                    &call_data.tokens,
                    gas,
                    &call_data.endpoint_name,
