...
use dharitri_sc_scenario::{
    imports::DrtscPath, managed_address, managed_biguint, rust_biguint, ExpectMessage,
...
const CODE_PATH: DrtscPath = DrtscPath::new("output/multisig.drtsc.json");
...
fn test_transfer_execute_to_user() {
    ...
    world.check_account(MULTISIG).balance(rewa_amount);
    ...
    world.check_account(new_user).balance(rewa_amount);
}
...
fn test_transfer_execute_sc_all() {
    ...
    let adder_code_path: DrtscPath = DrtscPath::new("test-contracts/adder.drtsc.json");
...
fn test_deploy_and_upgrade_from_source() {
    ...
    let adder_owner: TestAddress = TestAddress::new("adder-owner");
    let adder_address: TestSCAddress = TestSCAddress::new("adder");
    let adder_path: DrtscPath = DrtscPath::new("test-contracts/adder.drtsc.json");
    ...
    let factorial_code: DrtscPath = DrtscPath::new("test-contracts/factorial.drtsc.json");
